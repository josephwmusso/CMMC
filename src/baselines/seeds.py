...
Seed two hand-curated security baselines into the shared catalog:
  - CIS Microsoft Windows 11 Enterprise L1 v3.0  (~25 items)
  - CIS Microsoft 365 Foundations L1 v3.1        (~20 items)

Called from scripts/render_startup.py after tables exist. Idempotent:
skips work if the ``baselines`` table is already populated.
"""
...
WIN11_ITEMS = [
    # (cis_id, section, title, expected_value, severity, control_ids, match_keywords, match_plugin_families)
...
def _insert_baseline(cur, baseline_id: str, name: str, version: str,
    ...
    for row in items:
        (cis_id, section, title, expected_value, severity,
         control_ids, match_keywords, match_plugin_families) = row
        cur.execute(
            _INSERT_ITEM,
            (
...
def seed_baselines(cur) -> None:
    """Seed baselines + baseline_items if the baselines table is empty.

    Accepts a psycopg2 cursor so this slots into the existing
    render_startup.py seed sequence. Caller owns the commit.
    """
    cur.execute("SELECT COUNT(*) FROM baselines")
    existing = cur.fetchone()[0]
    if existing > 0:
        logger.info(f"  baselines: {existing} already seeded, skipping")
        return
    ...
    # Fill item_count from the actual row count.
    cur.execute("""
        UPDATE baselines b
    ...
    logger.info(f"  baselines: seeded {baselines_ct} baselines, {items_ct} items")
